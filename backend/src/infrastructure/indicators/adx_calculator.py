...
class ADXCalculator:
    ...
    def calculate_adx(
        ...
        plus_dm_list, minus_dm_list = self._calculate_directional_movements(candles)
        
        # Step 2: Calculate true ranges
        true_ranges = self.atr_calculator._calculate_true_ranges(candles)
        ...
        if len(candles) >= calc_period * 3:
            adx_value = self._calculate_adx_with_smoothing(
                candles, calc_period, plus_dm_list, minus_dm_list, true_ranges
            )
