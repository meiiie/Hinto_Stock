...
class ExecutionSimulator:
    ...
    def place_order(self, signal: TradingSignal):
        ...
        if self.mode == "SHARK_TANK":
            if len(self.positions) + len(self.pending_orders) >= self.max_positions: return
        ...
        # Hard Leverage Cap & Liquidity Cap (REALITY CHECK)
        max_notional_leverage = self.balance * self.max_leverage
        ...
        if notional < 10: return # Min order size $10
    ...
    def _execute_fill(self, order, price, time, slippage):
        ...
        if self.fixed_leverage > 0:
            effective_leverage = self.fixed_leverage
        ...
        self.logger.debug(f"🚀 FILLED {order['symbol']} {order['side']} @ {fill_price:.2f} | Liq: {liq_price:.2f}")
    ...
    def _liquidate_position(self, symbol, price, time):
        ...
        effective_leverage = 10.0
        if self.fixed_leverage > 0: effective_leverage = self.fixed_leverage
        
        margin_lost = pos['notional_value'] / effective_leverage
        
        self.balance -= margin_lost
        
        self._record_trade(pos, price, -margin_lost, "LIQUIDATION", time, pos['remaining_size'])
